...
public partial class CheckoutController : ControllerBase
{
    ...
    protected virtual async Task<IActionResult> EditAddressAsync(AddressModel addressModel, IFormCollection form, Func<Customer, IList<ShoppingCartItem>, Address, Task<JsonResult>> getResult)
    {
        try
        {
            ...
            if (customAttributeWarnings.Any())
                return BadRequest(new { error = 1, message = customAttributeWarnings });
        ...
        catch (Exception exc)
        {
            ...
            return BadRequest(new { error = 1, message = exc.Message });
        }
    }

    protected virtual async Task<IActionResult> DeleteAddressAsync(int addressId, Func<IList<ShoppingCartItem>, Task<JsonResult>> getResult)
    {
        try
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
    ...
    //Need to check later if required how to implement the same  Europium SOIlutions

    //[HttpPost("SaveEditBillingAddress")]
    //public async Task<IActionResult> SaveEditBillingAddress(CheckoutBillingAddressModel model, IFormCollection form, bool opc = false)
    //{
    ...
    //        if (!opc)
    //            return BadRequest(new { redirect = Url.RouteUrl("CheckoutBillingAddress") });
    ...
    //            update_section = billingAddressModel
    //        });
    //    });
    //}
    ...
    /// <param name="addressId"></param>
    /// <param name="opc"></param>
    /// Need to check later Europium SOIlutions
    ...
    /// <param name="opc"></param>
    /// <returns></returns>
    /// Need to check later Europium SOlutions
    ...
    protected virtual async Task<IActionResult> OpcLoadStepAfterPaymentMethod(IPaymentMethod paymentMethod, IList<ShoppingCartItem> cart)
    {
        if (paymentMethod.SkipPaymentInfo ||
            (paymentMethod.PaymentMethodType == PaymentMethodType.Redirection && _paymentSettings.SkipPaymentInfoStepForRedirectionPaymentMethods))
        {
        ...
        return Ok(new
        {
            ...
            {
                name = "payment-info",
                paymenInfoModel = paymenInfoModel
            },
            goto_section = "payment_info"
        });
    }

    [HttpGet("OnePageCheckout")]
    ...
    [HttpPost("OpcSaveShippingMethod")]
    ...
    [HttpGet("OpcCompleteRedirectionPayment")]
